    ...
    switch (this.isClicked) {
      case true: {
        this.renderer.addClass(
          this.hostElement.nativeElement.parentNode,
          "isActive",
        );
        break;
      }
...
Pada directive `custom-directive` ini, ketika terdapat event `click`, maka akan mentoggle sebuah class _isActive_, kamu bisa menggunakan fungsi `@HostListener()` untuk menghandle events yang ada pada element directive ini.
...
export class SelectingElComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    const findInputEl = this.hostElement.nativeElement.querySelector(
      'input[type="checkbox"]',
    );
    const findAllBtnEl =
      this.hostElement.nativeElement.querySelectorAll("button");
    const hasClass =
      this.hostElement.nativeElement.classList.contains("container");

    console.group(findInputEl, findAllBtnEl, hasClass);
  }
