...
> _"Closure is a mechanism by which an inner function will have access to the variables defined in its outer function’s lexical scope even after the outer function has returned."_

Sampai disini paham? singkatnya _closures_ adalah sebuah inner function yang mempunyai akses untuk mengambil suatu nilai diseluruh scopenya baik diluar maupun didalam.
...
Terdapat 2 parameter disini, yaitu `a`, `b` dimana masing-masing mempunyai tipe data numeric, lalu dibagian `trait` adalah sebuah _function interfaces_, tempat untuk inisialisasi function. Traits ini mirip seperti [typescript interfaces](https://www.typescriptlang.org/docs/handbook/interfaces.html).
