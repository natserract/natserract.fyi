...
Pada case kali ini, saya hanya ingin memanggil fungsi `fn()` ini hanya sekali melalui event _onClick_

```ts
type Ref<T> = React.MutableRefObject<T>;

const callFnOnce =
  (fn: (...fnArgs: any) => void, executedRef: Ref<boolean>) =>
  (value: boolean) => {
